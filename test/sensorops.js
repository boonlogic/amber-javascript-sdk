    ...
    context('getStatus', function () {
        it('should get status', async function () {
            try {
                ...
                expect(response.pca,'pca type').to.be.a('Array')
                expect(response.pca,'pca length').to.have.lengthOf(73)
                expect(response.clusterGrowth,'clusterGrowth type').to.be.a('Array')
                expect(response.clusterGrowth,'clusterGrowth length').to.have.lengthOf(73)
                expect(response.clusterSizes,'clusterSizes type').to.be.a('Array')
                expect(response.clusterSizes,'clusterSizes length').to.have.lengthOf(73)
                expect(response.anomalyIndexes,'anomalyIndexes type').to.be.a('Array')
                expect(response.anomalyIndexes,'anomalyIndexes length').to.have.lengthOf(73)
                expect(response.frequencyIndexes,'frequenceIndexes type').to.be.a('Array')
                expect(response.frequencyIndexes,'frequenceIndexes length').to.have.lengthOf(73)
                expect(response.distanceIndexes,'distanceIndexes type').to.be.a('Array')
                expect(response.distanceIndexes,'distanceIndexes length').to.have.lengthOf(73)
                expect(response.totalInferences,'totalInferences').to.equal(115306)
                expect(response.numClusters,'numClusters').to.equal(73)
                expect(response.anomalyThreshold,'anomalyThreshold').to.equal(750)
                expect(response.state,'state').to.equal('Monitoring')
            } catch (error) {
                assert.fail(error)
            }
        })
